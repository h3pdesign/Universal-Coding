...
def fetch_raindrops_to_tag() -> tuple[Dict[int, Dict[str, Any]], Dict[int, int]]:
    """
    Fetch Raindrop articles with fewer than 3 tags and calculate tag statistics.

    Returns:
        tuple[Dict[int, Dict[str, Any]], Dict[int, int]]: 
            - Dictionary of raindrop IDs to raindrop data for items with < 3 tags
            - Dictionary with tag count statistics {tag_count: number_of_articles}
    """
    logging.info("Fetching Raindrop articles with fewer than 3 tags...")
    all_raindrops: Dict[int, Dict[str, Any]] = {}
    tag_count_stats: Dict[int, int] = defaultdict(int)
    total_articles = 0
    page = 0
    ...
    while True:
        ...
        try:
            ...
            # Update total count for this page
    ...
    untagged_count = tag_count_stats.get(0, 0)
    under_three_count = sum(tag_count_stats.get(i, 0) for i in range(3))
    

    logging.info(f"Fetched {len(all_raindrops)} raindrops needing tags.")
    logging.info(f"Tag statistics - Total articles: {total_articles}, Untagged: {untagged_count}, Under 3 tags: {under_three_count}")
    
    return all_raindrops, tag_count_stats
...
def main() -> None:
    ...
    try:
        ...
        # Fetch raindrops to tag and get tag statistics
        raindrops, tag_stats = fetch_raindrops_to_tag()
        
        # Print tag statistics
        total_articles = sum(tag_stats.values())
        untagged_count = tag_stats.get(0, 0)
        one_tag_count = tag_stats.get(1, 0)
        two_tags_count = tag_stats.get(2, 0)
        under_three_count = untagged_count + one_tag_count + two_tags_count
        
        print(f"Raindrop Tag Statistics:")
        print(f"-------------------------")
        print(f"Total articles: {total_articles}")
        print(f"Untagged articles: {untagged_count} ({untagged_count/total_articles*100:.1f}%)")
        print(f"Articles with 1 tag: {one_tag_count} ({one_tag_count/total_articles*100:.1f}%)")
        print(f"Articles with 2 tags: {two_tags_count} ({two_tags_count/total_articles*100:.1f}%)")
        print(f"Articles with fewer than 3 tags: {under_three_count} ({under_three_count/total_articles*100:.1f}%)")
        print(f"-------------------------")
