...
def validate_username(
    ...
    if username in ignore_users:
        return "---> {} is in ignore_users list, skipping " "user...".format(username)
    ...
    if followers > like_by_followers_upper_limit:
        ...
        return "---> {}, number of followers does not reach " "minimum".format(username)
...
def update_activity(action=None):
    ...
    with conn:
        ...
        if data is None:
            ...
            cur.execute("INSERT INTO statistics VALUES " "(0, 0, 0, 0, 1, date('now'))")
        else:
            ...
            sql = (
                "UPDATE statistics set likes = ?, comments = ?, "
...
def get_active_users(browser, username, posts, logger):
    ...
    total_posts = format_number(
        browser.find_element_by_xpath("//span[contains(@class,'_t98z6')]//span").text
    )
    ...
    for count in range(1, posts):
        try:
            browser.find_element_by_xpath("//a[contains(@class, '_nzn1h')]").click()
            sleep(1)
            tmp_list = browser.find_elements_by_xpath("//a[contains(@class, '_2g7d5')]")
        except NoSuchElementException:
            try:
