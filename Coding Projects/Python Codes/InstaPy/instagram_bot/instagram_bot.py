...
# Set a logger cache outside the InstagramBot object to avoid re-instantiation issues
loggers = {}


class InstagramBotError(Exception):
    """General error for InstagramBot exceptions"""
...
class InstagramBot:
    ...
    def __init__(
        ...
        if os.name == "nt":
            error_msg = (
                "Sorry, Record Activity is not working on Windows. "
    ...
    def get_instapy_logger(self, show_logs):
        ...
        if existing_logger is not None:
            ...
            # initialize and setup logging system for the InstagramBot object
    ...
    def set_selenium_local_session(self):
        ...
        if self.use_firefox:
            ...
            try:
                ...
                raise InstagramBotError(
                    "ensure chromedriver is installed at {}".format(
            ...
            if float(matches.groups()[0]) < Settings.chromedriver_min_version:
                raise InstagramBotError(
                    "chromedriver {} is not supported, expects {}+".format(
        ...
        self.logger.info(
            "Session started - %s" % (datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        )
    ...
    def set_use_clarifai(self, enabled=False, api_key=None, full_match=False):
        ...
        Raises:
            InstagramBotError if os is windows
        """
        if self.aborting:
            return self

        if os.name == "nt":
            raise InstagramBotError("Clarifai is not supported on Windows")
    ...
    def follow_by_list(self, followlist, times=1):
        ...
        for acc_to_follow in followlist:
            ...
            if self.follow_restrict.get(acc_to_follow, 0) < self.follow_times:
                ...
                self.logger.info(
                    "---> {} has already been followed more than "
                    "{} times".format(acc_to_follow, str(self.follow_times))
                )
                sleep(1)
    ...
    def like_by_locations(
        self, locations=None, amount=50, media=None, skip_top_posts=True
    ):
                    ...
                    if not inappropriate:
                        ...
                        if liked:
                            ...
                            if self.use_clarifai and (following or commenting):
                                try:
                                    checked_img, temp_comments = check_image(
                                        self.browser,
                            ...
                            if (
                                ...
                                and checked_img
                                and following
                                and self.follow_restrict.get(user_name, 0)
                                < self.follow_times
                            ):
                        ...
                        self.logger.info("--> Image not liked: {}".format(reason))
                        inap_img += 1
            ...
            for i, link in enumerate(links):
                ...
                try:
                    ...
                    if not inappropriate:
                        ...
                        self.logger.info(
                            "--> Image not liked: Likes are disabled for method 'comment_by_locations'"
                        )
        ...
        for index, tag in enumerate(tags):
            ...
            try:
                links = get_links_for_tag(
                    self.browser, tag, amount, self.logger, media, skip_top_posts
                )
            except NoSuchElementException:
                        ...
                        if liked:
                            ...
                            if interact:
                                username = self.browser.find_element_by_xpath(
                                    "//article/header/div[2]/" "div[1]/div/a"
                                )
                                ...
                                self.logger.info("--> User followed: {}".format(name))
                                self.like_by_users(
                                    name,
        ...
        for index, username in enumerate(usernames):
            ...
            try:
                links = get_links_for_username(
                    self.browser, username, amount, self.logger, randomize, media
                )
            except NoSuchElementException:
                ...
                try:
                    ...
                    if not inappropriate:
                        ...
                        if (
                            ...
                            and username not in self.dont_include
                            and following
                            and self.follow_restrict.get(username, 0)
                            < self.follow_times
                        ):
    ...
    def interact_user_followers(self, usernames, amount=10, randomize=False):
        ...
        try:
            ...
            if isinstance(err, RuntimeWarning):
                self.logger.warning("Warning: {} , stopping follow_users".format(err))
                return self
            else:
                self.logger.error("Sorry, an error occured: {}".format(err))
                self.aborting = True
    ...
    def follow_user_followers(
        self, usernames, amount=10, randomize=False, interact=False, sleep_delay=600
    ):
        ...
        if interact:
            ...
            userFollowed = random.sample(
                userFollowed,
                int(ceil(self.user_interact_percentage * len(userFollowed) / 100)),
            )
            self.like_by_users(
                userFollowed,
                self.user_interact_amount,
    ...
    def unfollow_users(
        ...
        if unfollow_after is not None:
            if not python_version().startswith(("2.7", "3")):
                self.logger.info(
                    "`unfollow_after` argument is not available for Python versions below 2.7"
                )
                unfollow_after = None

        if onlyInstapyFollowed:
            self.automatedFollowedPool = set_automated_followed_pool(
                self.username, self.logger, self.logfolder, unfollow_after
            )

        try:
            unfollowNumber = unfollow(
                self.browser,
                self.username,
                amount,
                self.dont_include,
            ...
            self.logger.info("--> Total people unfollowed : {} ".format(unfollowNumber))
            self.unfollowNumber += unfollowNumber

        except (TypeError, RuntimeWarning) as err:
            if isinstance(err, RuntimeWarning):
                self.logger.warning("Warning: {} , stopping unfollow_users".format(err))
                return self
            else:
    ...
    def like_by_feed_generator(
        self, amount=50, randomize=False, unfollow=False, interact=False
    ):
        """Like the users feed"""
    ...
    def set_dont_unfollow_active_users(self, enabled=False, posts=4):
        ...
        active_users = get_active_users(self.browser, self.username, posts, self.logger)
    ...
    def end(self):
        ...
        self.logger.info(
            "Session ended - {}".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        )
        self.logger.info("-" * 20 + "\n\n")
