...
def test_raindrop_connection():
    try:
        response = requests.get(
            RAINDROP_API_URL, headers=HEADERS, params={"perpage": 1}, timeout=10
        )
        response.raise_for_status()
        data = response.json()
        print(
            f"Test fetch successful: Retrieved {len(data.get('items', []))} raindrop(s) on first page"
        )
        return True
    except Exception as e:
...
def main():
    ...
    if total_raindrops is not None and total_tags is not None:
        ...
        raindrops_with_no_tags = sum(
            1 for a in all_raindrops.values() if not a.get("tags")
        )
        print(f"Number of raindrops with no tags: {raindrops_with_no_tags}")
    else:
        print(
            "Failed to fetch Raindrop stats. Check the error message above for details."
        )
