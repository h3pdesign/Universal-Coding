...
def get_links_from_feed(browser, amount, num_of_search, logger):
    ...
    # get links
    link_elems = browser.find_elements_by_xpath("//article/div[2]/div[2]/a")
...
def get_links_for_tag(browser, tag, amount, logger, media=None, skip_top_posts=True):
    """Fetches the number of links specified
    by amount and returns a list of links"""
    if media is None:
        # All known media types
    ...
    browser.get(
        "https://www.instagram.com/explore/tags/" + (tag[1:] if tag[:1] == "#" else tag)
    )
    # update server calls
    update_activity()
    sleep(2)
...
def get_links_for_username(
    browser, username, amount, logger, randomize=False, media=None
):
    """Fetches the number of links specified
    by amount and returns a list of links"""
    if media is None:
...
def check_link(
    ...
    if like_by_followers_upper_limit or like_by_followers_lower_limit:
        ...
        if (
            like_by_followers_upper_limit
            and num_followers > like_by_followers_upper_limit
        ):
            return True, user_name, is_video, "Number of followers exceeds limit"

        if (
            like_by_followers_lower_limit
            and num_followers < like_by_followers_lower_limit
        ):
            return (
                True,
                user_name,
    ...
    for dont_likes_regex in dont_like_regex:
        ...
        if quash:
            ...
            iffy = (
                ...
                if dont_likes_regex.endswith("*([^\\d\\w]|$)")  # 'word' without format
                else (
                    (re.split(r"\W+", dont_likes_regex))[1]
                    if dont_likes_regex.endswith("+([^\\d\\w]|$)")  # '[word'
                    else (
                        (re.split(r"\W+", dont_likes_regex))[3]
                        if dont_likes_regex.startswith("#[\\d\\w]+")  # ']word'
                        else (re.split(r"\W+", dont_likes_regex))[1]
                    )
                )
            )  # '#word'
            inapp_unit = 'Inappropriate! ~ contains "{}"'.format(
                quashed if iffy == quashed else '" in "'.join([iffy, quashed])
            )
...
def like_image(browser, username, blacklist, logger, logfolder):
    ...
    spans = [
        x.text.lower()
        for x in browser.find_elements_by_xpath("//article//a[@role='button']/span")
    ]
