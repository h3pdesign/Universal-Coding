...
----------------------------------------------------------------------------
  ...
  Description:
    ...
    A simple python app to primarily display current arrivals and departures, given an airports' IATA code (3 letters). However, it can also list all API available airports given a country name or airport details given an IATA code, and more..
...
def usage():
    ...
    print(
        "        -i <airport-iata>    -- Show detailed information about an airport given its IATA code"
    )
    print(
    ...
    print(
        "                                (This option is getting raw METAR info from:  aviationweather.gov )"
    )
    print(
        "        -f <flight/id>       -- Get in-air information (speed,alt,bearing etc) given a certain flight number or id"
    )
    print(
        "        -r <lat,lon> <radius> -- Show flights/airplanes within the given radius [km] of a decimal lat/lon location."
    )
    print(
        "        -t                   -- Show arrival/departure times in the local timezone (TZ) of the airport"
    )
    print(
        "        -w                   -- Show the weather for the Airport given by its <iata> code"
    )
...
if not opts:
    ...
    for opt, arg in opts:
        ...
        #    apicreds = 1; apiuser = args[0]; apipass = args[1];
            ...
            query = 2
            airport = arg
            # Show Departures from <iata>
        elif opt == "-i":
            query = 4
            airport = arg
            # Show airport INFO from <iata>
        elif opt == "-j":
            debug = int(arg)
            # FIX:  airport = args[0]; country = args[0];
        elif opt == "-l":
            query = 3
            country = arg
            # Show list of <iata> codes from <country>
        elif opt == "-m":
            query = 5
            airport = arg.upper()
            # Show last hour's METAR data for an airport given by <airport-code>
        elif opt == "-n":
            nmax = int(arg)
            # airport = args[0]                     # FIX:  Maximum number of output items/pages
        # elif opt == "-t": loctz = True;                        # Show all results in local airport TimeZone
        elif opt == "-x":  # Show flights between 2 airports <iata-1> <iata-2>
            query = 6
            airport = arg
            airport2 = args[0].upper()
...
if query == 1:
    if nmax == 0:
        alist = api.get_airport_arrivals(airport)
        # Return: a JSON dictionary of xxxxx
    else:
    ...
    if nmax == 0:
        alist = api.get_airport_departures(airport)
        # Return: a JSON dictionary of xxxxx
    else:
    ...
    try:
        alist = api.get_airports(country)
        # Return: a list of (name,code) tuples. # api.get_airports('India')[10:15]
    ...
    try:
        alist = api.get_airport_details(airport)
        # Return:
        blist = api.get_airport_weather(airport)
        # Return:
    except AttributeError:
        iataerr()
    except TypeError:
    ...
    try:
        alist = api.get_flights_from_to(airport, airport2)
        # Return: a JSON dictionary of xxxxx
    except AttributeError:
        Print("Attribute exception")
        # airerr();
    if not alist:
        print("empty list - no conncting flights?")
        airerr()
...
if query == 1:
    ...
    HLine = "ID\t Flight\t From\t Sched\t ETA\t %s\t %s" % (
        "Airline".ljust(24, " "),
        "Status".ljust(15, " "),
    )
    ...
    for aL in alist:
        ...
        if status == "Scheduled":
            status = "-"
        if re.match(r"[Dd]elayed", status):
            status = yellow(status)
        if re.match(r"[Cc]anceled", status):
            status = red(status)
        if re.match(r"[Ll]anded", status):
            status = green(status)
        airline = airlinN[:24].ljust(24, " ")

        # FLine = ("%s\t %s\t %s\t %s\t %s\t %s\t %s"% (callsig, flight1, origin, hs(timeS), hs(timeE), airline, status) )
        FLine = "{}\t {}\t {}\t {}\t {}\t {}\t {}".format(
            callsig, flight1, origin, hs(timeS), hs(timeE), airline, status
        )
        print(FLine)
...
if query == 2:
    ...
    # HLine = ("ID\t Flight\t From\t Sched\t ETA\t Airline\t Status") #  callsig, flight1, origin, hs(timeS), hs(timeE), airlinN, status
    HLine = "ID\t Flight\t To\t Sched\t ETA\t %s\t %s" % (
        "Airline".ljust(24, " "),
        "Status".ljust(15, " "),
    )
    Hleng = len(HLine) + 6 * 4  # also count 4-space tab lengths
...
if query == 4:
    ...
    if True:
        ...
        print(
            "Visibility [{}]:\t{}".format(visi_unit, round(skyvisi, 1))
        )  # Need to truncate decimals
...
if query == 6:
    ...
    HLine = "{:<8}  {:<8} {:<16}  {:<5}  {:<24} {:<20} {:<19}".format(
        *headers
    )  # * = unpack the tuple
    Hleng = len(HLine)  # + 6*4  # also count 4-space tab lengths
    ...
    for aL in alist:
        ...
        if (
            aL["airport"]["destination"] == "None"
        ):  # Some destinations doesn't have an IATA code or is unknown
            goingto = "---"
        else:
        ...
        # headers = ('ID', 'Flight', 'Scheduled', 'ETD', 'Airline', 'Aircraft', 'Status')
        FLine = "{:<8}  {:<8} {:<16}  {:<5}  {:<24} {:<20} {:<19}".format(
            callsig, flight1, hd(timeS), hs(timeE), airline, aircraft, status
        )  # changed from hs() to hd()
