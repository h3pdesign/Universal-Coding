...
def scrape(origin, destination, startdate, days, requests):
    ...
    chrome_options.add_argument(
        "--user-agent=" + agents[(requests % len(agents))] + '"'
    )
    ...
    if (
        soup.find_all("p")[0].getText()
        == "Please confirm that you are a real KAYAK user."
    ):
        print(
            "Kayak thinks I'm a bot, which I am ... so let's wait a bit and try again"
        )
        driver.close()
        time.sleep(20)
        return "failure"
...
results_agg = (
    results.groupby(["destination", "startdate"])["price"]
    .min()
    .reset_index()
