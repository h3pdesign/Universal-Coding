...
def test_readwise_connection():
    try:
        response = requests.get(
            READWISE_LIST_URL, headers=HEADERS, params={"pageCursor": None}, timeout=10
        )
        response.raise_for_status()
        data = response.json()
        print(
            f"Test fetch successful: Retrieved {len(data.get('results', []))} article(s) on first page"
        )
        return True
    except Exception as e:
...
def main():
    ...
    if total_articles is not None and total_tags is not None:
        ...
        articles_with_no_tags = sum(
            1 for a in all_articles.values() if not a.get("tags")
        )
        print(f"Number of articles with no tags: {articles_with_no_tags}")
    else:
        print(
            "Failed to fetch Readwise stats. Check the error message above for details."
        )
